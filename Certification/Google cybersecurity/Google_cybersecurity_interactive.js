document.addEventListener('DOMContentLoaded', () => {
  ...
  const revealBlock = (block, animate = true) => {
    ...
    const headings = block.querySelectorAll('.page-title, .page-subtitle, .section-title');
    let headDelay = animate ? 0 : 0;
    headings.forEach((el) => {
      el.classList.add('is-visible');
      el.style.animationDelay = `${headDelay}s`;
      headDelay += animate ? 0.1 : 0;
    });

    const texts = block.querySelectorAll('.section-text');
    let textDelay = animate ? headDelay + 0.12 : 0;
    texts.forEach((p) => {
      p.classList.add('is-visible');
      p.style.animationDelay = `${textDelay}s`;
      textDelay += animate ? 0.25 : 0;
    });
  };
